...
def fit_linear_regression(X, y):
    ...
    """
    # Notice that for a design matrix defined as defined in class transposes are not needed
...
def predict(X, w):
    ...
    """
    # Notice that for a design matrix defined as defined in class transpose is needed
    return X @ w
...
def plot_singular_values(X):
    ...
    ggsave(width=5, height=5, verbose=False, filename="singular.values.scree.plot.png", plot=(
        ggplot(pd.DataFrame({'x': range(len(sv)), 'y': sv}), aes(x='x', y='y')) +
        geom_point() +
        labs(title="Scree Plot of Design Matrix Singular Values", x="", y="Singular Value")
    ))
...
def feature_evaluation(X, y):
    ...
    for f in X:
        ...
        ggsave(width=5, height=5, verbose=False, filename="pearson.correlation.%s.png" % f, plot=(
            ggplot(pd.DataFrame({'x': X[f], 'y': y}), aes(x='x', y='y')) +
            geom_point() +
            geom_smooth(colour="blue", fill="grey", method='lm') +
            labs(title=("Correlation Between %s Values and Response\n" % f)+r"Pearson Correlation $\rho=%.4f$" % rho,
                 x=f + " Values", y="Response Values")))
...
if __name__ == "__main__":
    ...
    ggsave(width=5, height=5, verbose=False, filename="mse.over.training.percentage.png", plot=
        ggplot(pd.DataFrame({'x': range(1, 101), 'y': results})) +
         geom_line(aes(x='x', y='y')) +
         labs(x="Percentage of Training Set", y="MSE Over Test Set",
              title="Model Evaluation Over Increasing Portions Of Training Set"))
